...
def crossValidation(fold, trial):
    """This method conducts cross validate for bayesbest, the functions used in this method are almost
    the same as bayesbest, but with some changes to be suitable for performance measure
    The reason why to separate crossValidation function from bayes class is using the member functions in
    bayesbest will overwrite the bayes model we need
    ...
    for _, trailValue in confusionMat_trial.items():
        for _, foldValue in trailValue.items():
            total += 1
            try:
                accuracy = float(foldValue["truePositive"] + foldValue["trueNegative"]) / float(
                    sum(foldValue.values()))
            except ZeroDivisionError:
                accuracy = 0.0
            try:
                precision_pos = float(foldValue["truePositive"]) / float(
                    foldValue["truePositive"] + foldValue["falsePositive"])
                precision_neg = float(foldValue["trueNegative"]) / float(
                    foldValue["trueNegative"] + foldValue["falseNegative"])
            except ZeroDivisionError:
                precision_pos = 0.0
                precision_neg = 0.0
            try:
                recall_pos = float(foldValue["truePositive"]) / float(
                    foldValue["truePositive"] + foldValue["falseNegative"])
                recall_neg = float(foldValue["trueNegative"]) / float(
                    foldValue["trueNegative"] + foldValue["falsePositive"])
            except ZeroDivisionError:
                recall_pos = 0.0
                recall_neg = 0.0
            try:
